...
from src.fahe1 import dec1, length_enc1, keygen1
from src.fahe2 import dec2, enc2, keygen2
import time
from src.plotting import plot_performance, plot_ciphertext_size
...
class TestFAHE1GraphKeyGenLambdaPerformance(unittest.TestCase):
    ...
    def test_data_collection_timed_keygen1(self):
        ...
        data_points = collect_security_param_performance(
            2, 100, 2, Operation.TIMED_KEYGEN, self.operation_map, params
        )
        print_data_points(data_points)
        plot_performance(
            data_points,
            "Key Generation Time Performance vs. Lambda",
            "Lambda",
            "Time Performance (millisec)",
        )
...
class TestFAHE1GraphKeyGenAlphaPerformance(unittest.TestCase):
    ...
    def test_data_collection_timed_keygen1(self):
        ...
        data_points = collect_alpha_performance(
            2, 100, 2, Operation.TIMED_KEYGEN, self.operation_map, params
        )
        print_data_points(data_points)
        plot_performance(
            data_points,
            "Key Generation Time Performance vs. Alpha",
            "Alpha",
            "Time Performance (millisec)",
        )
...
class TestFAHE1GraphEncryptLambdaPerformance(unittest.TestCase):
    ...
    def test_data_collection_timed_encrypt_(self):
        ...
        data_points = collect_security_param_performance(
            64, 256, 8, Operation.TIMED_ENCODE, self.operation_map, params
        )
        print_data_points(data_points)
        plot_performance(
            data_points,
            "Encrypt Time Performance vs. Lambda",
            "Lambda",
            "Time Performance (millisec)",
        )
...
class TestFAHE1GraphEncryptLambdaCiphertext(unittest.TestCase):
    ...
    def test_data_collection_ciphertext(self):
        ...
        plot_ciphertext_size(
            data_points,
            "Encrypt Ciphertext Size vs. Lambda",
            "Lambda",
            "Ciphertext Size",
        )
